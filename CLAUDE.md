...
- Cascata de nomes: pasta `<fonte>/`, script `<fonte>_etl.py`, source YAML = chave de `ETLS` = `<entidade>`, schema `raw_<fonte>` (chave `db_schema` no topo do YAML), tabela `<entidade>`. Deixe no fim do `__main__` o comentário com o comando `uv run python -m ... [entidade ...] [--steps ...]`.
