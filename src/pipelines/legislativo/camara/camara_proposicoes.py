...
logger = logging.getLogger("raw_camara_proposicao")

_CONFIG_FILE = Path(__file__).parent / "camara_config.yml"
_SEM_DADOS_FILE = "sem_dados_id_proposicao.csv"
_LANDING_SUFFIX = "_proposicao"
# Extrai o id da proposicao a partir da url self (".../proposicoes/{id}").
_PROP_ID_RE = re.compile(r"/proposicoes/([^/]+)")
...
def extract(cfg: PipelineConfig):
    ...
    ids_landing = pd.DataFrame(
        {
            "id": [
                f.stem.removesuffix("_proposicao")
...
def _parse_file(f: Path) -> pd.DataFrame | None:
    ...
    # Campos de texto da API (ementa, keywords, despacho...) trazem CRLF interno.
    # Sem isso, o \r vira quebra de linha nao-citada no CSV e estoura o parser C
    # do pandas no load ("Buffer overflow caught"). Normaliza para espaco unico.
...
def _processed_prop_ids(bronze_path: Path) -> set[str]:
    """Ids de proposicao ja presentes no bronze, lidos da coluna urls em chunks.
    ...
    usado (inexistente, vazio ou sem a coluna urls), sinalizando rebuild.
    """
    ids: set[str] = set()
    try:
        for chunk in pd.read_csv(
            bronze_path, sep=";", usecols=["urls"], chunksize=200_000, dtype=str
        ):
            for url in chunk["urls"].dropna().unique():
...
def transform(cfg: PipelineConfig):
    ...
    urls do proprio bronze) e faz append atomico via copia temporaria. Se o
