"""Votos individuais por votação (extração incremental por ID, bronze em streaming)."""
...
def transform(cfg: PipelineConfig) -> None:
    write_bronze_streaming(
        ...
        partial(parse_dados_abertos, url_col="url_votos"),
    )


def build() -> GenericETL:
    cfg = PipelineConfig.from_yaml(_CONFIG_FILE, "votos_deputados")
    return GenericETL(
